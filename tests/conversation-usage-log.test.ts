...
beforeEach(async () => {
  ...
  await saveConfig({
    ...config,
    activeAccountId: `conversation-mode-account-${crypto.randomUUID()}`,
    usageLogCountMode: "conversation",
  })
...
describe("conversation usage log refresh", () => {
  test("refreshes upstream usage for every repeated request in the same conversation", async () => {
    const sessionId = `conversation-session-${crypto.randomUUID()}`
  ...
  test("refreshes upstream usage again when the conversation variant changes", async () => {
    const sessionId = `conversation-session-${crypto.randomUUID()}`
    const accountId = mergeConfigWithDefaults().activeAccountId
